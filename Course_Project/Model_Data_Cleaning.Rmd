...
model_data_num <- model_data %>% select_if(is.numeric)
model_data_non_num <- model_data %>% select_if(~!is.numeric(.x))


# Use preProcess to imput missing values using KNN model, **May automatically apply centering and scaling?
pre_Imputed_model_Data <- preProcess(model_data_num, method = c("bagImpute"))
imputed_model_Data <- predict(pre_Imputed_model_Data, model_data_num)

model_data_bagImputed<- cbind(model_data_non_num, imputed_model_Data)

model_data_bagImputed<- subset(model_data_bagImputed, select = -Company_Creation_Date)
