...
trControl <- trainControl(method = 'cv', number = 10, savePredictions = TRUE, classProbs = TRUE, sampling = "smote", search = 'grid')
...
xgb_churn_tree_smote <- train(as.formula(model_form), data = data_train, method = model_type, trControl = trControl, metric = 'ROC')

xgb_churn_tree$finalModel

# get training data predictions
xgb_churn_tree_smote_predictions <- xgb_churn_tree_smote$pred

# evaluate predictions
confusionMatrix(xgb_churn_tree_smote_predictions$pred, xgb_churn_tree_smote_predictions$obs)
...
roc(xgb_churn_tree_smote_predictions$obs, xgb_churn_tree_smote_predictions$Yes, plot = TRUE, print.auc = TRUE, legacy.axes = TRUE, levels = c(negative_class, positive_class))$auc
